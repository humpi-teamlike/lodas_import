...
Aufruf:
  python lodas_beta_v4_1.py
"""

VERSION = "lodas_import_v4"
...
unbezahlt_schluessel = set(
    r["schluessel"] for r in lohnarten_regeln
    if r["ziel"] == "Fehlzeiten"
    and r["schluessel"] is not None
    and r["fehlzeit_key"] == "22"
)
...
if unbezahlt_schluessel:
    log.info(f"  Unbezahlte Fehlzeit Schluessel: {unbezahlt_schluessel}")
...
def verarbeite_kommentar_block(schluessel, zeilen_sorted, pnr,
    ...
    if schluessel in unbezahlt_schluessel:
        unbezahlt_tage += len(zeilen_sorted)
    ...
    return auszahlungsstunden, unbezahlt_tage
...
def verarbeite_zeitnachweis(datei_pfad, datei_name):
    ...
    if ist_gehalt:
        ...
        gehalt_schluessel = {"Krank", "Kind krank", "Urlaub"}
        ...
        for quelle in (kommentar_tage, abwesenheiten_tage):
            for schluessel, zeilen in quelle.items():
                zeilen_sorted = sorted(zeilen, key=lambda x: datetime.strptime(x["datum"], "%d.%m.%Y"))
                auszahlungsstunden, unbezahlt_tage = verarbeite_kommentar_block(
                    schluessel, zeilen_sorted, pnr,
                    ist_stunden_pro_tag, soll_stunden_pro_tag,
                    ergebnis, auszahlungsstunden, direkt_buchungen, unbezahlt_tage)
        ...
        if normale_tage:
            gesamt_stunden_la100 = sum(z["stunden"] for z in normale_tage)
        ...
        if normale_tage or unbezahlt_tage > 0:
            abzug       = unbezahlt_tage * soll_stunden_pro_tag
            la100_netto = round(gesamt_stunden_la100 - abzug, 2)
            if unbezahlt_tage > 0:
                report.hinweis(name, f"Unbezahlte Fehlzeit: {unbezahlt_tage} Tage x {soll_stunden_pro_tag}h SOLL = -{abzug:.2f}h Abzug von LA 100")
        ...
        report.add_ma(
            name              = name,
            pnr               = pnr,
            la100_h           = la100_netto if (normale_tage or unbezahlt_tage > 0) else None,
            azk_la            = azk_la,
            azk_h             = azk_h,
            fehlzeiten_anzahl = len(ergebnis["fehlzeiten"]),
            saldo_differenz   = saldo_differenz,
