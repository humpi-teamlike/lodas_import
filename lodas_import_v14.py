"""
LODAS Importskript  lodas_import_v3
====================================
    ...
    for regel in regeln:
        if regel["ziel"] == "Fehlzeiten":
            ...
            if regel["einheit"] == "Stunden":
                if regel["la_nr"] == "123":
                    gesamt = len(zeilen_sorted) * soll_stunden_pro_tag
...
def verarbeite_zeitnachweis(datei_pfad, datei_name):
    ...
    for zeile in daten_zeilen:
        ...
        if k and k.lower().replace(" ", "") == "-sollfeiertag":
            log.info(f"  -SOLL Feiertag ignoriert: {zeile['stunden']:.2f}h am {zeile['datum']}")
            continue
    ...
    if ist_gehalt:
        ...
        if normale_tage:
            # SOLL-Deckelung: pro Tag maximal SOLL-Stunden auf LA 100
            gedeckelt_summe = 0.0
            deckelung_tage = 0
            deckelung_diff = 0.0
            for z in normale_tage:
                gedeckelt = min(z["stunden"], soll_stunden_pro_tag)
                diff = z["stunden"] - gedeckelt
                gedeckelt_summe += gedeckelt
                if diff > 0:
                    deckelung_tage += 1
                    deckelung_diff += diff
            gesamt_stunden_la100 = round(gedeckelt_summe, 2)
            if deckelung_tage > 0:
                report.hinweis(name, f"LA 100 SOLL-Deckelung: {deckelung_tage} Tag(e) mit IST > SOLL, {deckelung_diff:.2f}h gedeckelt (fliessen ins AZK)")
                log.info(f"  LA 100 SOLL-Deckelung: {deckelung_tage} Tag(e), {deckelung_diff:.2f}h gedeckelt")
        ...
        ueberstunden = 0.0
        if normale_tage:
            ueberstunden = round(sum(z["stunden"] - soll_stunden_pro_tag for z in normale_tage), 2)
            log.info(f"  Ueberstunden-Berechnung: {len(normale_tage)} Tage, "
                     f"IST {sum(z['stunden'] for z in normale_tage):.2f}h - "
                     f"SOLL {len(normale_tage) * soll_stunden_pro_tag:.2f}h = {ueberstunden:+.2f}h")

        # FZA (LA 123): verbrauchte AZK-Stunden abziehen
        fza_abzug = round(sum(
            safe_float(bwd["wert"]) for bwd in ergebnis["bewegungsdaten"] if bwd["la_nr"] == "123"
        ), 2)

        # AZK-Kommentar-Buchungen (z.B. Garantiezeit LA 210, negativ gebucht = AZK+)
        azk_kommentar = round(sum(
            abs(safe_float(bwd["wert"])) for bwd in ergebnis["bewegungsdaten"]
            if bwd["la_nr"] not in ("122", "123", "100") and safe_float(bwd["wert"]) < 0
        ), 2)

        azk_netto = round(ueberstunden - fza_abzug + azk_kommentar, 2)
        log.info(f"  AZK-Netto: Ueberstunden {ueberstunden:+.2f}h - FZA {fza_abzug:.2f}h "
                 f"+ AZK-Kommentar {azk_kommentar:.2f}h = {azk_netto:+.2f}h")

        azk_la = None; azk_h = 0.0
        if azk_netto > 0:
            if la122:
                ergebnis["bewegungsdaten"].append(
                    bwd_satz(pnr, abrechnungszeitraum_global, f"-{azk_netto:.2f}", la122["bs_nr"] or "1", "122"))
                azk_la = "122"; azk_h = azk_netto
                log.info(f"  AZK-Buchung: LA 122 -{azk_netto:.2f}h")
        elif azk_netto < 0:
            if la124:
                ergebnis["bewegungsdaten"].append(
                    bwd_satz(pnr, abrechnungszeitraum_global, f"{abs(azk_netto):.2f}", la124["bs_nr"] or "1", "124"))
                azk_la = "124"; azk_h = abs(azk_netto)
                log.info(f"  AZK-Buchung: LA 124 +{abs(azk_netto):.2f}h")
        else:
            log.info("  -> Kein AZK-Ausgleich notwendig (AZK-Netto = 0)")

        # Kontroll-Hinweis: Eigenberechnung vs. Leitsystem-Saldo
        if saldo_differenz is not None:
            report.hinweis(name, f"AZK-Kontrolle: Eigen {azk_netto:+.2f}h, Leitsystem-Saldo {saldo_differenz:+.2f}h")
