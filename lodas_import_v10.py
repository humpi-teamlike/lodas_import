...
Aufruf:
  python lodas_import_v10.py
"""

VERSION = "lodas_import_v10"
...
def verarbeite_zeitnachweis(datei_pfad, datei_name):
    ...
    if ist_gehalt:
        ...
        # AZK-Buchung aus Saldo-Differenz (v2-Logik)
        azk_la = None; azk_h = 0.0
        if saldo_differenz is not None and saldo_differenz != 0.0:
            if saldo_differenz > 0:
                if la122:
                    ergebnis["bewegungsdaten"].append(
                        bwd_satz(pnr, abrechnungszeitraum_global, f"-{saldo_differenz:.2f}", la122["bs_nr"] or "1", "122"))
                    azk_la = "122"; azk_h = saldo_differenz
                    log.info(f"  AZK-Buchung: LA 122 -{saldo_differenz:.2f}h (Saldo gestiegen)")
                    # Rundungshinweis: gebuchter Wert vs. Saldo-Differenz
                    if round(saldo_differenz, 2) != round(azk_h, 2):
                        report.hinweis(name, f"AZK LA 122: Saldo-Differenz {saldo_differenz:.2f}h, gebucht {azk_h:.2f}h – Rundungsabweichung {abs(saldo_differenz - azk_h):.2f}h")
            elif saldo_differenz < 0:
                if la124:
                    ergebnis["bewegungsdaten"].append(
                        bwd_satz(pnr, abrechnungszeitraum_global, f"{abs(saldo_differenz):.2f}", la124["bs_nr"] or "1", "124"))
                    azk_la = "124"; azk_h = abs(saldo_differenz)
                    log.info(f"  AZK-Buchung: LA 124 +{abs(saldo_differenz):.2f}h (Saldo gesunken)")
                    # Rundungshinweis: gebuchter Wert vs. Saldo-Differenz
                    if round(abs(saldo_differenz), 2) != round(azk_h, 2):
                        report.hinweis(name, f"AZK LA 124: Saldo-Differenz {abs(saldo_differenz):.2f}h, gebucht {azk_h:.2f}h – Rundungsabweichung {abs(abs(saldo_differenz) - azk_h):.2f}h")
        elif saldo_differenz == 0.0:
            log.info("  -> Kein AZK-Ausgleich notwendig (Saldo-Differenz = 0)")
