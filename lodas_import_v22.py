"""
LODAS Importskript  lodas_import_v22
====================================
Liest Zeitnachweis-Excel-Dateien aus einem Monatsordner und erzeugt
eine LODAS-ASCII-Importdatei.

Neu in v22 (Mai 2026):
  - SOLL-Validierung: Gehaltsempfaenger ohne Zeitnachweis-Datei im
    Lohnexport-Ordner werden jetzt ebenfalls in der Gehaltsempfaenger-
    Liste unterhalb der Abgleich-Tabelle ausgefuehrt.
    Bisher wurden nur Gehaltsempfaenger gelistet, fuer die eine xlsx-Datei
    vorhanden war. Gehaltsempfaenger ohne Zeitnachweis (z. B. bei
    unveraendertem Gehalt ohne Abwesenheiten) blieben unsichtbar.
    Neu: Nach der Schleife ueber Zeitnachweise wird zusaetzlich aus
    mitarbeiter_map geprueft, welche Gehaltsempfaenger noch nicht erfasst
    wurden, und diese als "(kein Zeitnachweis)" angehaengt.

Neu in v21 (Mai 2026):
  - Gehaltsempfaenger werden in der SOLL-Validierung uebersprungen und
    separat unterhalb der Abgleich-Tabelle aufgelistet (Gehaltsempfaenger).
    Kein SOLL-Abgleich moeglich, aber visuelle Kontrollmoeglichkeit bleibt.

Neu in v20 (Mai 2026):
  - SOLL-Validierung als erster Schritt nach Pfad-Eingabe (BLOCK 6b).
    Fuer jeden Zeitnachweis im Ordner wird das Tages-SOLL unabhaengig
    berechnet und mit dem hinterlegten Wert in stammdaten.xlsx verglichen.
    Berechnung:
      1. Monats-SOLL  = Betrag der Differenz aus Zeile "Soll-Stunden" in der
                        Uebersicht des Zeitnachweis-Sheets.
      2. SOLL-Tage    = eindeutige Werktage (Mo-Fr) aus Zeitnachweis-Sheet
                        + FZA/Urlaubs-Tage aus Abwesenheiten-Sheet (Set-Union)
                        - Feiertage (Zeilen mit Kommentar "Feiertag").
      3. Tages-SOLL   = Monats-SOLL / SOLL-Tage
    Ausgabe als Tabelle. Benutzer entscheidet ob Import fortgesetzt wird.
...
# ===========================================================================
# BLOCK 6b – SOLL-Validierung (vor dem Import)
# ===========================================================================

def berechne_soll_aus_zeitnachweis(datei_pfad):
    """
    Berechnet das Tages-SOLL unabhaengig aus dem Zeitnachweis.
    Gibt (tages_soll_berechnet, name, fehler_msg) zurueck.

    Formel:
      monats_soll = Betrag der "Soll-Stunden"-Differenz aus der Uebersicht
      soll_tage   = eindeutige Werktage im Zeitnachweis-Sheet
                    + FZA/Urlaub-Tage aus Abwesenheiten-Sheet (Set-Union)
                    - Feiertage (Kommentar "Feiertag")
      tages_soll  = monats_soll / soll_tage
    """
    try:
        df_raw = pd.read_excel(datei_pfad, sheet_name="Zeitnachweis", header=None)
    except Exception as e:
        return None, None, f"Zeitnachweis-Sheet nicht lesbar: {e}"

    # Name lesen
    try:
        name_raw = str(df_raw.iloc[0, 2]).strip()
        teile = name_raw.split(",", 1)
        name = f"{teile[0].strip()}, {teile[1].strip()}" if len(teile) == 2 else name_raw
    except Exception:
        name = "Unbekannt"
    ...
    if header_idx is None:
        return None, name, "Kein Datum-Header gefunden"
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            continue
        if dt.month != az_monat or dt.year != az_jahr:
            continue
        if dt.weekday() >= 5:
            continue  # Sa/So ignorieren (z.B. Garantiezeit am Wochenende)
        stunden_raw = row.get("Stunden")
        try:
            stunden = 0.0 if pd.isna(stunden_raw) else float(stunden_raw)
        except (TypeError, ValueError):
            stunden = 0.0
        if stunden < 0:
            continue  # SOLL-Gegenbuchungen ignorieren
        kommentar = clean(row.get("Kommentar") or "")
        zn_werktage.add(dt.date())
        if "feiertag" in kommentar.lower():
            feiertag_daten.add(dt.date())

    # FZA und Urlaub aus Abwesenheiten-Sheet (Set-Union)
    abw_daten = set()
    try:
        xl = pd.ExcelFile(datei_pfad)
        if "Abwesenheiten" in xl.sheet_names:
            raw_abw = pd.read_excel(datei_pfad, sheet_name="Abwesenheiten", header=None)
            header_abw = None
            for i, row in raw_abw.iterrows():
                if any("Beginn" in str(v) for v in row.values):
                    header_abw = i
                    break
            if header_abw is not None:
                raw_abw.columns = [norm_col(c) for c in raw_abw.iloc[header_abw]]
                df_abw = raw_abw.iloc[header_abw + 1:].reset_index(drop=True)
                for _, row in df_abw.iterrows():
                    art = clean(row.get("Art") or "")
                    if art.lower() not in ("freizeitausgleich", "urlaub"):
                        continue
                    ...
                    dt = dt_von
                    while dt <= dt_bis:
                        if dt.month == az_monat and dt.year == az_jahr and dt.weekday() < 5:
                            abw_daten.add(dt.date())
                        dt += timedelta(days=1)
    except Exception:
        pass

    alle_soll_tage = (zn_werktage | abw_daten) - feiertag_daten

    if not alle_soll_tage:
        return None, name, "Keine SOLL-Tage ermittelbar"

    # Monats-SOLL aus Uebersicht lesen
    monats_soll   = None
    in_uebersicht = False
    for i, row in df_raw.iterrows():
        ...
        if not in_uebersicht:
            if label in ("uebersicht", "ubersicht", "übersicht"):
                in_uebersicht = True
            continue
        if "soll" in label and "stunden" in label:
            werte = [
            ...
            if len(werte) >= 3:
                monats_soll = abs(werte[2])
                break
            elif len(werte) >= 2:
                monats_soll = abs(round(werte[1] - werte[0], 4))
                break

    if monats_soll is None or monats_soll == 0:
        return None, name, "Monats-SOLL aus Uebersicht nicht lesbar"

    tages_soll = round(monats_soll / len(alle_soll_tage), 4)
    return tages_soll, name, None


def soll_validierung(xlsx_dateien, zeitnachweis_ordner):
    """
    Laeuft ueber alle Zeitnachweise, berechnet Tages-SOLL und vergleicht
    mit stammdaten.xlsx. Gibt Tabelle auf Konsole aus.
    Gehaltsempfaenger werden separat aufgelistet.
    Gibt True zurueck wenn der Benutzer fortfahren moechte, sonst False.
    """
    SEP  = "=" * 75
    SEP2 = "-" * 75
    ...
    print()
    print(SEP)
    print(f"  SOLL-Validierung  [{VERSION}]")
    print(SEP)
    print(
        f"  {'Mitarbeiter':<28} {'SOLL berechn.':>14} {'SOLL Stammd.':>13} "
        f"{'Delta':>9}  {'Status':<10}"
    )
    print("  " + SEP2)

    irgendein_fehler = False
    gehalt_liste     = []

    for datei in sorted(xlsx_dateien):
        pfad = os.path.join(zeitnachweis_ordner, datei)

        tages_soll_berechnet, name, fehler_msg = berechne_soll_aus_zeitnachweis(pfad)

        # Gehaltsempfaenger: separat sammeln, kein Abgleich
        ma = mitarbeiter_map.get(name) if name else None
        if ma and ma.get("ist_gehalt"):
            gehalt_liste.append(name)
            continue

        if fehler_msg or tages_soll_berechnet is None:
            label = name or datei
            print(f"  {label:<28} {'---':>14} {'---':>13} {'---':>9}  FEHLER: {fehler_msg or 'Unbekannt'}")
            irgendein_fehler = True
            continue

        if ma is None:
            print(f"  {name:<28} {tages_soll_berechnet:>13.4f}h {'---':>13} {'---':>9}  NICHT IN STAMMDATEN")
            irgendein_fehler = True
            continue

        soll_stammdaten = ma["soll"]
        delta           = round(tages_soll_berechnet - soll_stammdaten, 4)
        ...
        print(
            f"  {name:<28} {tages_soll_berechnet:>13.4f}h {soll_stammdaten:>12.4f}h "
            f"{delta:>+9.4f}h  {status}"
        )

    print("  " + SEP2)
    if irgendein_fehler:
        print("  HINWEIS: Es wurden Abweichungen oder Fehler festgestellt.")
    else:
        print("  Alle SOLL-Werte stimmen mit den Stammdaten ueberein.")

    # Gehaltsempfaenger ohne Zeitnachweis: aus mitarbeiter_map hinzufuegen
    for ma_name, ma_data in mitarbeiter_map.items():
        if ma_data.get("ist_gehalt") and ma_name not in gehalt_liste:
            gehalt_liste.append(f"{ma_name}  (kein Zeitnachweis)")

    if gehalt_liste:
        print()
        print("  Gehaltsempfaenger (kein SOLL-Abgleich):")
        for g in gehalt_liste:
            print(f"    💼  {g}")

    print(SEP)
    print()

    antwort = input("  Import fortsetzen? [j/n]: ").strip().lower()
    if antwort != "j":
        print("  Abbruch durch Benutzer. Import wird nicht ausgefuehrt.")
        return False
    print()
    return True
...
if not soll_validierung(xlsx_dateien, zeitnachweis_ordner):
    exit(0)
